...
const CardBeritaSmall = ({ id, img, judul, deskripsi, jenis }) => {
    ...
    return (
        <div className='w-[10rem] h-[21rem] flex flex-col justify-start items-start overflow-hidden gap-4 shrink-0 md:w-[17rem] md:h-[34rem] md:mr-4 lg:h-[26rem] lg:w-[19rem] lg:gap-2'>
            <div className='w-full h-[8rem] bg-black rounded-2xl overflow-hidden shrink-0 md:h-[13rem] lg:mb-2 lg:rounded-4xl lg:h-[12rem]'>
                <img src={`${import.meta.env.VITE_API_BASE_IMG_URL}/news/${img}`} alt="" className='w-full h-full object-cover' />
            </div>
            ...
            <Link to={`/berita-detail/${id}`} className='text-xs underline cursor-pointer active:text-slate-400 md:text-lg lg:text-xs'>
                Baca Selengkapnya
            </Link>
        </div>
    )
