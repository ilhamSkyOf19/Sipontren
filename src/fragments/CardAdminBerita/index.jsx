...
const CardAdminBerita = ({ img, judul, jenis }) => {
    ...
    return (
        <div className='w-[10rem] h-[24rem] flex flex-col justify-start items-start overflow-hidden gap-4 shrink-0 md:w-[17rem] md:h-[34rem] md:mr-4 lg:h-[26rem] lg:w-[19rem] lg:gap-2'>
            ...
            <div className='w-full flex flex-row justify-start items-center gap-2 flex-wrap'>
                ...
                {/* update button */}
                <ButtonCrud color={'rgba(0, 0, 0, 0.7)'} link={'berita/add'} size={'py-2 px-2.5'}>
                    Update
                </ButtonCrud>

                {/* delete button */}
                <button type='button' className='text-sm py-2 px-3 bg-red-600 rounded-lg text-white active:bg-red-500' onClick={() => confirmDelete(1)}>Delete</button>
